...
create_btrfs_snapshot() {
    ...
    # Check if source subvolume exists
    [ ! -d "$src_subvol" ] && error_exit "Source subvolume $src_subvol does not exist."
    
    # Create snapshot directory if it does not exist
    [ ! -d "$snapshot_dir" ] && mkdir -p "$snapshot_dir"
    
    # Create the snapshot
    btrfs subvolume snapshot "$src_subvol" "$snapshot_dir/$snapshot_name" || error_exit "Failed to create snapshot."
}


# Create a Btrfs snapshot before backup
snapshot_name="snapshot_$(date +%Y%m%d%H%M%S)"
create_btrfs_snapshot "$SOURCE_DIR" "$BACKUP_DIR/snapshots" "$snapshot_name"
...
# Btrfs-related Configuration
# Directory to store Btrfs snapshots
export SNAPSHOT_DIR="$BACKUP_DIR/snapshots"
# Number of snapshots to retain during rotation
export NUM_SNAPSHOTS_TO_RETAIN=5
