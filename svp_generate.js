...
function defineSmooth()
{
    ...
    if (global.gpu>0)
    {
        ...
        if(profile.nvof && profile.nvof_quality<2)
            smooth.nvof.q = profile.nvof_quality;
    }
...
{
    var frc = false;
    if(!profile.on || (media.fi_num===1 && media.fi_den===1) || global.crop_preview || profile.rife)
    {
        levels={};
        analyse={};
        smooth={rate:{num:1}};
    ...
    smooth.hdr = {};
    if(global.gpu>0 && media.hdr_peak>0)
...
{
    while(profile.nvof_grid>4 && (media.dst_w/profile.nvof_grid < 40 || media.dst_h/profile.nvof_grid < 32))
        switch(profile.nvof_grid)
        {
        case 32: profile.nvof_grid = 24; break;
        case 24: profile.nvof_grid = 16; break;
        default: profile.nvof_grid /= 2;
        }
...
function gen_avs_prepare_clip()
{
    res = '';
    bl = '\t'; br = '\r\n';

    res += bl+'input_um = src'+br;
    res += bl+'input_m = input_um'+br;

    if(media.crop && !global.crop_preview)
        res += bl+"input_m = input_um.Crop("+media.crop[0]+","+media.crop[1]+",-"+media.crop[2]+",-"+media.crop[3]+")"+br;

    if(media.scale && !global.crop_preview)
    {
        if(!media.scale_down)
            res += bl+"input_m = input_m.LanczosResize("+media.scale[0]+","+media.scale[1]+")"+br;
        else
            res += bl+"input_m = input_m.BicubicResize("+media.scale[0]+","+media.scale[1]+",b=0,c=0.75)"+br;
    }

    res += bl+'input_m8 = '+(media.p10 ? 'input_m.ConvertBits(8)':'input_m')+br;
    if(profile.nvof)
        res += bl+'input_m8 = input_m8.BicubicResize(input_m.width/nvof_blk*4,input_m.height/nvof_blk*4,src_width=-(input_m.width % nvof_blk),src_height=-(input_m.height % nvof_blk))'+br;
...
function gen_avs(is_avsplus)
{
    ...
    {
	case 3: srcClip = "potplayer_source"; break;
	case 4: srcClip = "AvsFilterSource"; break;
	default:srcClip = "ffdshow_source"; break;
    }
    ...
    if(is_avsplus)
    {
        ...
        AVS.push('SetFilterMTMode("'+srcClip+'",3)');
    }
    ...
    if(is_avsplus)
    {
        AVS.push("ConvertToYUV420()");
        if(global.player===4)
            AVS.push("ConvertBits(" + (media.p10 === true ? 16:8) + (media.dither>0 ? (",dither="+(media.dither-1)):"")+")");
    }
    else AVS.push("ConvertToYV12()");
    AVS.push("");
    ...
    AVS.push("");
    AVS.push(global.baseScript.replace("#RESIZE-CODE",gen_avs_prepare_clip()));
    AVS.push("");
...
function gen_vs_prepare_clip()
{
    ...
    dither = media.dither<=1 ? "random":"error_diffusion";
    if(global.player===2 && media.codec==="rv32") //VLC + RGB input
        res += bl+'input_um = clip.resize.Bicubic(format=vs.YUV420P8,matrix_s="709",dither_type="'+dither+'")'+br;
    else
...
function gen_vs()
{
    ...
    if(!profile.rife && !profile.nvof)
    {
    	VS.push("if not hasattr(core,'svp1'):");
    	VS.push('    core.std.LoadPlugin("'+apppath+pluginsDir+sep+pfx+'svpflow1'+sfx+'")');
    }
    VS.push("if not hasattr(core,'svp2'):");
    VS.push('    core.std.LoadPlugin("'+apppath+pluginsDir+sep+pfx+'svpflow2'+sfx+'")');
    if(profile.rife) 
    {
	VS.push("if not hasattr(core,'rife'):");
	VS.push('    core.std.LoadPlugin("'+apppath+'rife'+sep+pfx+'RIFE'+sfx+'")');
    }
    VS.push("");
    ...
    switch(global.player)
    {
	case 4: srcClip = "VpsFilterSource"; break;
	default:srcClip = "video_in"; break;
    }

    if(profile.fi_duplicates===1)
        VS.push("clip = core.std.SelectEvery("+srcClip+",2,0)");
    else VS.push('clip = '+srcClip);
    ...
    //VS.push("rife = "+(profile.rife ? 1:0));
    if(profile.rife)
    {
        VS.push("rife_model = " + profile.rife_model);
        VS.push("rife_precision = " + (profile.rife_precision ? "True":"False"));
        VS.push("rife_tta = " + (profile.rife_tta ? "True":"False"));
        VS.push("rife_threads = " + profile.rife_threads);
        VS.push("rife_gpu = " + profile.rife_gpu);
    }
...
//        VS.push('    demo_clip = core.std.StackHorizontal([core.assvapour.Subtitle(half_src,"Source")[0],core.std.BlankClip(half_src,width=2),core.assvapour.Subtitle(half_smooth,"Smooth")[0]])');
    ...
    if(global.player===2) //VLC
    {
        if(media.codec==="rv32") //RGB input
            ...
            VS.push('smooth = smooth.resize.Bilinear(format=vs.'+media.src_fmt+')');
    }
